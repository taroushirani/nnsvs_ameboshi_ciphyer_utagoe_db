...
full_score_dir = join(config["out_dir"], "utaupy_full_round_seg")
...
for base in tqdm(base_files):
    ...
    lab_score_path = join(config["out_dir"], "utaupy_full_round", f"{utt_id}.lab")
...
black_list_file_path = join(config["out_dir"], "timelag_black.list")
with open(black_list_file_path, "w", encoding="UTF-8") as of :
    of.write("\n".join(black_list))
...
black_list_file_path = join(config["out_dir"], "duration_black.list")
with open(black_list_file_path, "w", encoding="UTF-8") as of :
    of.write("\n".join(black_list))
...
for base in tqdm(base_files):
    ...
    while True:
        ...
        wav_slice = wav[b:e]
        if config["suppress_start_end_pau"]:
            print("Enable start&end pau suppression")
            regexp = re.compile("-pau+")
            if re.search(regexp, lab_align[0][2]):
                d = int((lab_align[0][1] - lab_align[0][0]) * 1e-7 * config["start_end_pau_suppression_ratio"] * sr)
                print(f"First pau of {name} is suppressed for {(d/sr):2f} sec.")
                wav_slice[0:d] = 0
            if re.search(regexp, lab_align[-1][2]):
                d = int((lab_align[-1][1] - lab_align[-1][0]) * 1e-7 *  config["start_end_pau_suppression_ratio"] * sr)
                print(f"Last pau of {name} is suppressed for {(d/sr):2f} sec")
