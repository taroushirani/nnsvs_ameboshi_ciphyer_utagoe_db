...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    sh $script_dir/local/data_prep.sh ./config.yaml
    mkdir -p data/list

    echo "train/dev/eval split"
    find data/acoustic/ -type f -name "*.wav" -exec basename {} .wav \; \
        | grep -v -f data/timelag_black.list | grep -v -f data/duration_black.list | sort > data/list/utt_list.txt
    grep akatonbo data/list/utt_list.txt > data/list/$eval_set.list
    grep alps_ichimanjaku data/list/utt_list.txt > data/list/$dev_set.list
